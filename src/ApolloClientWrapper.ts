...
import type { SessionSocket } from "./api/resources/session/client/Socket.js";
...
export class ApolloClient extends _GeneratedClient {
     ...
     * Authorization on the upgrade (the generated resource only forwards per-call
     * headers, so the client-level auth header has to be injected here).
     */
    async connect(args: ApolloClient.ConnectArgs = {}): Promise<SessionSocket> {
        ...
        return this.session.connect({ ...args, headers });
    }
}
